...
- Forced global `scroll-behavior: auto !important` under `prefers-reduced-motion`, so role-selection scrolling cannot inherit the site-wide smooth-scroll rule.
...
- `node --test apps/web/src/lib/campaign-copy.test.mjs apps/web/src/lib/campaign-form-state.test.mjs apps/web/src/lib/campaign-phone-contract.test.mjs apps/web/src/lib/campaign-leads.test.mjs apps/web/src/lib/campaign-routing.test.mjs` — 27/27 pass (copy/notice linkage, fresh role state and consents, last-touch attribution reset, contact-shaped attribution rejection, web↔shared/backend phone fixtures, strict durable funnel DTO/receipt/once/no-PII behavior, `formStartedAt`, credential-free lead submission, safe API error semantics, fail-closed auth/public mode, minimal shell routing, and global reduced-motion override proof).
