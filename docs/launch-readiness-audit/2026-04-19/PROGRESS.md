...
### 2026-04-22 — WS-11 Payments And Earnings Policy

- Added explicit offline payment confirmation to booking updates in `packages/schemas/src/dto.ts` and `apps/backend/src/modules/bookings/bookings.service.ts`, with provider/admin-only authorization and a completed-booking guard.
- Reconciled earnings with booking payment state: completing an unpaid job creates a pending earning with an explanatory note, while confirming payment settles the existing earning transaction instead of leaving unpaid work looking available.
- Added focused backend coverage in `apps/backend/src/modules/bookings/bookings.service.spec.ts` for unpaid completion, completion-with-payment, and post-completion payment confirmation.
