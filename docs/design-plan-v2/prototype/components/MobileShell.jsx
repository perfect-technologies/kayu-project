...
function MobileTabBar({ activeScreen, nav }) {
  const tabs = [
    ...
    { id: "bookings", label: "Réservations", icon: I.calendar, target: "bookings" },
    { id: "messages", label: "Messages",  icon: I.inbox,       target: "messages" },
    { id: "provider", label: "Mon espace pro", icon: I.sparkles, target: "provider" },
  ];
  const bookingLike = activeScreen === "booking";
  const computed = activeScreen === "profile" ? "search"
                 : activeScreen === "requests" || activeScreen === "earnings" ? "provider"
                 : activeScreen;
