...
| 2026-09-20 | Opus 5 answers on the gateway again; the `AGENT_MODEL_ID` override stays in place until the owner decides | The 429 that made Phase 1 add the override is gone as of today. Every real turn in Phases 1, 2 and 3 ran on Sonnet 5, so no transcript in this document was produced by the RFC build model; the Phase 4 baseline can now be Opus 5 |
...
- (resolved 2026-09-20) Opus 5 access on the AI Gateway account. It answers again: the catalog lists `anthropic/claude-opus-5` and two live calls returned 200 (a one-token probe, then « Réponds en une phrase : quel modèle es-tu ? » → « Je suis Claude, un assistant d'IA créé par Anthropic. », finish `stop`, 54 tokens, $0.00081). Unlocked or transient capacity, unknown which. `apps/backend/.env` still carries `AGENT_MODEL_ID=anthropic/claude-sonnet-5`, so nothing changed by itself: drop the override to build on the RFC model at $5/$25 per million tokens, or keep it to develop at Sonnet's $2/$10 and let the Phase 4 eval pick production.
