...
import {
  LAUNCH_FUNNEL_VALIDATION_ERROR_CODES,
  LAUNCH_FUNNEL_VALIDATION_FIELDS,
  type ClientLeadTiming,
  type CreateClientLeadDtoType,
  type CreateLaunchFunnelEventDtoType,
  type CreateLaunchFunnelEventResponse,
  type CreateLaunchLeadResponse,
  type CreateProviderLeadDtoType,
  type LeadAttributionDtoType,
  type ProviderLeadExperienceBand,
} from "@kayu/schemas";

export type CampaignLeadType = "provider" | "client";

export type LeadAttributionSource = NonNullable<
  LeadAttributionDtoType["source"]
>;
export type LeadAttributionMedium = NonNullable<
  LeadAttributionDtoType["medium"]
>;
export type LeadAttribution = LeadAttributionDtoType;

type RawLeadAttribution = Partial<
  Record<keyof LeadAttribution, string | undefined>
>;

export type ProviderExperienceBand = ProviderLeadExperienceBand;
export type ClientTiming = ClientLeadTiming;
export type CreateProviderLeadRequest = CreateProviderLeadDtoType;
export type CreateClientLeadRequest = CreateClientLeadDtoType;
export type LeadAcceptedResponse = CreateLaunchLeadResponse;
...
export type CampaignEventName = CreateLaunchFunnelEventDtoType["eventName"];
...
export type LaunchFunnelEventRequest = CreateLaunchFunnelEventDtoType;
...
const FUNNEL_VALIDATION_FIELDS = new Set(LAUNCH_FUNNEL_VALIDATION_FIELDS);
...
export async function persistCampaignEvent(
  ...
  try {
    ...
    const receipt = (await response.json()) as Partial<CreateLaunchFunnelEventResponse>;
